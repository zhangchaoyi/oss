...
public class RootInterceptor implements Interceptor {
	private static Logger logger = Logger.getLogger(RootInterceptor.class);
	...
	public void intercept(Invocation invocation) {
		...
		if (cookie != null) {
			...
			if (!"".equals(username)) {
				...
				permission = as.validateRolePermission(roles, "root");
			}
			if(permission==true){
				invocation.invoke();
				return;
		...
		if (actionKey.startsWith("/api")) {
			if (cookie == null) {
				...
				logger.debug("<RootInterceptor>: cookie is null,permission denied");
				return;
			}
		...
		logger.debug("<RootInterceptor>: cookie is not null,permission denied");
