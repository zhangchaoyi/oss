DELIMITER |
DROP EVENT IF EXISTS egg_log.event_return_user |
CREATE EVENT IF NOT EXISTS egg_log.event_return_user
ON SCHEDULE EVERY 1 DAY STARTS '2016-10-29 03:00:00'
ON COMPLETION PRESERVE
DO

-- 每天凌晨三点执行统计 前一天 活跃/付费/非付费 回流用户
BEGIN
    -- 声明活跃 及用于遍历的变量
	...
	-- 定义统计的当天, 往后推七天/十四天/三十天 日期  以10-29 03:00:00 执行为例,统计 10-20 七日回流, 10-13 十四日回流, 09-27 三十日回流
	declare i integer;
    declare current_day date;
    declare current_next_day date;
    declare yesterday date;
    declare yesterday_pre date; 
    set yesterday = date_sub(curdate(), interval 1 day);
	set yesterday_pre = date_sub(yesterday, interval 1 day);
   
	-- 初始化变量 统计昨日 活跃/付费/非付费
    ...
    -- 前一天活跃玩家 10-28
    select count(distinct A.account) into active_user_ios from login A join device_info B on A.openudid = B.openudid where A.date =  yesterday and B.os = "iOS";
    select count(distinct A.account) into active_user_android from login A join device_info B on A.openudid = B.openudid where A.date =  yesterday and B.os = "android";
    select count(distinct A.account) into active_user_windows from login A join device_info B on A.openudid = B.openudid where A.date =  yesterday and B.os = "windows";
    -- 前一天付费玩家
    select count(distinct A.account) into paid_user_ios from log_charge A join create_role B on A.account = B.account join device_info C on B.openudid = C.openudid  where A.is_product = 1 and DATE_FORMAT(A.timestamp,'%Y-%m-%d') = yesterday and C.os = "iOS";
    select count(distinct A.account) into paid_user_android from log_charge A join create_role B on A.account = B.account join device_info C on B.openudid = C.openudid  where A.is_product = 1 and DATE_FORMAT(A.timestamp,'%Y-%m-%d') = yesterday and C.os = "android";
    select count(distinct A.account) into paid_user_windows from log_charge A join create_role B on A.account = B.account join device_info C on B.openudid = C.openudid  where A.is_product = 1 and DATE_FORMAT(A.timestamp,'%Y-%m-%d') = yesterday and C.os = "windows";
    -- 前一天非付费玩家
    select count(*) into nonpaid_user_ios from (select distinct account from login where date = yesterday) A left join (select distinct account from log_charge where is_product = 1 and DATE_FORMAT(timestamp,'%Y-%m-%d') = yesterday) B on A.account = B.account join create_role C on A.account = C.account join device_info D on C.openudid = D.openudid where B.account is null and D.os = "iOS";
	select count(*) into nonpaid_user_android from (select distinct account from login where date = yesterday) A left join (select distinct account from log_charge where is_product = 1 and DATE_FORMAT(timestamp,'%Y-%m-%d') = yesterday) B on A.account = B.account join create_role C on A.account = C.account join device_info D on C.openudid = D.openudid where B.account is null and D.os = "android";
    select count(*) into nonpaid_user_windows from (select distinct account from login where date = yesterday) A left join (select distinct account from log_charge where is_product = 1 and DATE_FORMAT(timestamp,'%Y-%m-%d') = yesterday) B on A.account = B.account join create_role C on A.account = C.account join device_info D on C.openudid = D.openudid where B.account is null and D.os = "windows";
    -- 插入前一天 活跃/付费/非付费  用户
	insert into return_user(num,date,type,os) values (active_user_ios,yesterday,"active","iOS");
    insert into return_user(num,date,type,os) values (active_user_android,yesterday,"active","android");
    insert into return_user(num,date,type,os) values (active_user_windows,yesterday,"active","windows");
   
    insert into return_user(num,date,type,os) values (paid_user_ios,yesterday,"paid","iOS");
    insert into return_user(num,date,type,os) values (paid_user_android,yesterday,"paid","android");
    insert into return_user(num,date,type,os) values (paid_user_windows,yesterday,"paid","windows");
    
    insert into return_user(num,date,type,os) values (nonpaid_user_ios,yesterday,"nonpaid","iOS");
    insert into return_user(num,date,type,os) values (nonpaid_user_android,yesterday,"nonpaid","android");
    insert into return_user(num,date,type,os) values (nonpaid_user_windows,yesterday,"nonpaid","windows");
    
    -- 循环执行 +1....+7.....+14,以10-29 03:00为例,计算10-26的+1日回流,10-27的+2日回流...10-20的+7日回流
    set i = 2;
    WHILE i < 16 DO
    BEGIN
		set current_day = date_sub(yesterday,interval i day);
        ...
        select count(*) into v_active_user_ios from (select distinct account from login A join device_info B on A.openudid = B.openudid where A.date = current_day and B.os = "iOS") A left join (select distinct A.account from login A join device_info B on A.openudid = B.openudid where A.date between current_next_day and yesterday_pre and B.os = "iOS") B on A.account = B.account join (select distinct account from login where date = yesterday) C on A.account = C.account where B.account is null; 
