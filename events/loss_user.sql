DELIMITER |
DROP EVENT IF EXISTS egg_log.event_loss_user |
CREATE EVENT IF NOT EXISTS egg_log.event_loss_user
ON SCHEDULE EVERY 1 DAY STARTS '2016-09-07 03:00:00'
ON COMPLETION PRESERVE
DO

-- 每天凌晨三点执行统计 前一天 活跃/付费/非付费 流失用户
BEGIN
    -- 声明活跃
    ...
    -- 定义统计的当天, 往后推一/二/三....七天/十四天/三十天 日期  以10-29 03:00:00 执行为例,统计10-27第一日日流失,10-26日第二日流失.... 10-21 七日流失, 10-14 十四日流失, 09-28 三十日流失
	...
	-- 插入前一天 活跃/付费/非付费  用户
	insert into loss_user(num,date,type,os) values (active_user_ios,yesterday,"active","iOS");
    insert into loss_user(num,date,type,os) values (active_user_android,yesterday,"active","android");
    insert into loss_user(num,date,type,os) values (active_user_windows,yesterday,"active","windows");
   
    insert into loss_user(num,date,type,os) values (paid_user_ios,yesterday,"paid","iOS");
    insert into loss_user(num,date,type,os) values (paid_user_android,yesterday,"paid","android");
    insert into loss_user(num,date,type,os) values (paid_user_windows,yesterday,"paid","windows");
    
    insert into loss_user(num,date,type,os) values (nonpaid_user_ios,yesterday,"nonpaid","iOS");
    insert into loss_user(num,date,type,os) values (nonpaid_user_android,yesterday,"nonpaid","android");
    insert into loss_user(num,date,type,os) values (nonpaid_user_windows,yesterday,"nonpaid","windows");
    
    -- 循环进行 更新+1 +2 +3 +4 +5....的数据
    set i = 1;
    WHILE i < 15 DO
    BEGIN
